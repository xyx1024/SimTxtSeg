...
class SE_Conv_Block(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_out=False):
        ...
        if planes <= 16:
            ...
            self.globalAvgPool = nn.AvgPool2d((96, 96), stride=1)    # (56, 75) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((96, 96), stride=1)
        elif planes == 192:
            self.globalAvgPool = nn.AvgPool2d((48, 48), stride=1)    # (28, 37) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((48, 48 ), stride=1)
        elif planes == 384 :
            self.globalAvgPool = nn.AvgPool2d((24, 24), stride=1)    # (14, 18) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((24, 24), stride=1)
    ...
    def forward(self, x):
        ...
        out = self.globalAvgPool(out)#[b,256,1,1]
